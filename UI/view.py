...
class View(ft.UserControl):
    ...
    def load_interface(self):
        ...
        self._ddAnno = ft.Dropdown(label="Anno", width=200, alignment=ft.alignment.top_left, on_change=self._controller.stampaSquad)
        ...
        self._ddSquadra = ft.Dropdown(label="Squadra", on_change= self._controller.abilitazione)
        self._btnDettagli = ft.ElevatedButton(text="Dettagli", on_click=self._controller.handleDettagli, disabled=True)
        self._btnPercorso = ft.ElevatedButton(text="Percorso", on_click=self._controller.handlePercorso, disabled=True)
